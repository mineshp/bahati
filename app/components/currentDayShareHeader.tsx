...
export default function CurrentDayShareHeader(prop: Props) {
  ...
  function extractStockName(data: StockData): string {
    return data.longName.substring(0, 15).replace(',', '').replace('Inc.', '').trim().split(' ')[0];
  }
  ...
  return (
    <div className={`grid grid-cols-3 p-2 bg-gray-200 border-2 rounded-lg bg-gradient-to-r ${randomBgHeader[randomHeader].bg}`}>
      ...
      <div className="..."><span className="items-center text-xs sm:text-sm text-sky-200">{extractStockName(data)}:{data.exchange}</span></div>
